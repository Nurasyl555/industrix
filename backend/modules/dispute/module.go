...
func NewModule(pg *postgres.Client, deals contracts.DealProvider, freezer contracts.DealFreezer,
	escrow contracts.EscrowSettler, events contracts.EventPublisher, notifier contracts.Notifier) *Module {
	repo := NewRepository(pg)
	svc := NewService(repo, deals, freezer, escrow, events, notifier)
